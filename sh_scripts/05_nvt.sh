...
#SBATCH --ntasks=1
#SBATCH --array=0-10
#SBATCH --output=slurm_logs/nvt_%a.log
...
mkdir -p "$LOG_DIR"
echo "Logging to: ${LOG_DIR}/nvt_${BASENAME}.log"
exec > "${LOG_DIR}/nvt_${BASENAME}.log" 2>&1
...
for i in $(seq 0 $((N_REPLICAS - 1))); do
    echo "--- Replica $i ---"
    python "$SCRIPT" --config "$JSON_FILE" --step nvt --replica "$i" --skip_if_done
done
